...
void gaussFilter(Mat& source, Mat& output1, int radius, double sigma) {
	if (sigma < 0) return;
...
int main(int argc, char** argv) {
	string sourceName = "../../imgs/source", outputName = "../../imgs/linoutput";
	int number;
	...
	try {
		...
		if (source.data != NULL) {
			...
			gaussFilter(source, output, RADIUS, sigma);
			double end_time = clock();
			double filter_time = (end_time - start_time) / CLOCKS_PER_SEC;
			cout << "LIN time = " << filter_time << endl;
